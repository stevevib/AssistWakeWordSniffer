...
namespace AssistWakeWordSniffer
{
    public class Program
    {
        public static async Task Main( string[] args )
        {
            ...
            // Pre-boot Setup: We need a temporary logger and settings object to run the loader
            ...
            // Load .env into Environment Variables and map to our settings object
            LoadEnvFromDisk( tempLogger, settings );

            var builder = Host.CreateApplicationBuilder( args );

            // Configuration & Settings Singleton
            // Bind again to ensure any standard appsettings.json or command line args are merged
            builder.Configuration.Bind( settings );
            builder.Services.AddSingleton( settings );

            builder.Logging.ClearProviders();
            // This handles the standard console internally but adds emojis to system logs.
            builder.Logging.AddProvider( new LifecyclePrettyLoggerProvider( settings ) );
            ...
            // 6. Early Validation & Sanity Checks (Remains the same)
